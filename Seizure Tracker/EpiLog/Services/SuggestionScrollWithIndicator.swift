        ...
        ZStack(alignment: .topTrailing) {
            ...
            ScrollView(.vertical, showsIndicators: false) {
                    ...
                    GeometryReader { geo in
                        Color.clear
                            .preference(
                                ...
                                value: (-geo.frame(in: .named("suggestions")).minY).safe
                            )
                    }
                    .frame(height: 0)
                ...
                .background(
                    GeometryReader { geo in
                        Color.clear
                            .onAppear { contentHeight = max(geo.size.height.safe, 1) }
                            .onChange(of: geo.size.height) { _, newValue in
                                contentHeight = max(newValue.safe, 1)
                            }
                    }
                )
            }
            ...
            .onPreferenceChange(ScrollOffsetKey.self) { offset in
                scrollOffset = offset.safe
            }
            .frame(height: height.safe)

            // ---- Indicator ----
            let trackH = max(height.safe, 1)
            let ch = max(contentHeight.safe, 1)

            if ch > trackH {
                let thumbH = max(24, trackH * (trackH / ch))      // safe
                let maxOffset = max(ch - trackH, 1)
                let progress = min(max((scrollOffset.safe / maxOffset).safe, 0), 1)
                let thumbY = ((trackH - thumbH) * progress).safe
